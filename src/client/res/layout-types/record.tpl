...
<div class="panel panel-<%= panel.style %><%= panel.label ? ' headered' : '' %><%= panel.tabNumber ? ' tab-hidden' : '' %><% if (panel.name) { %> {{#if hiddenPanels.<%= panel.name %>}} hidden{{/if}}<% } %> <%= panel.additionalClasses ?? '' %>"
  ...
  <div class="panel-body panel-body-form">
    ...
    <div class="row">
      ...
      %>
      <div class="cell <%= spanClass %> form-group<% if (cell.field) { %> {{#if hiddenFields.<%= cell.field %>}}hidden-cell{{/if}}<% } %><% if (cell.horizontalLabel) { %> horizontal-label<% } %>"
           data-name="<%= cell.field %>"
           tabindex="-1">
        <% if (!cell.noLabel) { %>
        <label class="control-label<% if (cell.field) { %> {{#if hiddenFields.<%= cell.field %>}}hidden{{/if}}<% } %><% if (cell.horizontalLabel) { %> inline<% } %>"
               data-name="<%= cell.field %>">
    ...
    %>
      ...
      <div class="column <%= spanClass %>">
        ...
        <div class="cell form-group<% if (cell.field) { %> {{#if hiddenFields.<%= cell.field %>}}hidden-cell{{/if}}<% } %><% if (cell.horizontalLabel) { %> horizontal-label<% } %>"
             data-name="<%= cell.field %>">
          <% if (!cell.noLabel) { %>
          <label class="control-label<% if (cell.field) { %> {{#if hiddenFields.<%= cell.field %>}}hidden{{/if}}<% } %><% if (cell.horizontalLabel) { %> inline<% } %>"
                 data-name="<%= cell.field %>">
