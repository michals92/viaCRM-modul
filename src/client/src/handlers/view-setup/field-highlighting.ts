import type FieldHighlighterType from 'viacrm:helpers/field-highlighter';
...
interface FieldView extends View {
	...
	highlightSuccess?: () => void;
	highlightWarning?: () => void;
	highlightDanger?: () => void;
	highlightInfo?: () => void;
	highlightPrimary?: () => void;
...
interface FieldHighlighterInstance {
	getLayoutName(view: HighlightingView): string;
	getWatchedFields(scope: string): string[];
	applyRecordHighlighting(
	...
	applyStyleToElement(element: JQuery, style: Record<string, string>, uniqueId: string): void;
}

interface HighlightingView extends View {
	model: Model & { name: string; id?: string };
	entityType?: string;
	fieldHighlighter?: FieldHighlighterInstance;
	getMetadata(): Metadata;
	getUser(): User;
	getFieldViews?(): Record<string, FieldView>;
	getFieldView?(fieldName: string): FieldView | null;
	listenTo(target: unknown, event: string, callback: (...args: unknown[]) => void): void;
	on(event: string, callback: (...args: unknown[]) => void): void;
	highlightField?: (fieldName: string) => void;
	clearFieldHighlighting?: (fieldName: string) => void;
	applyFieldStyle?: (fieldName: string, style: Record<string, string>) => void;
	getFieldHighlightStyle?: (fieldName: string, directRules?: unknown) => Record<string, string> | null;
}
...
define(
	['viacrm:helpers/field-highlighter'],
	(FieldHighlighter: typeof FieldHighlighterType) => {
		/**
		...
		return class FieldHighlightingHandler {
			...
			highlightingCache: Map<string, boolean> = new Map();
			// Track watched field values to detect actual changes
			watchedFieldValues: Record<string, unknown> = {};
			// Track current highlighting state to prevent unnecessary DOM updates
			currentHighlightingState: Map<string, boolean> = new Map();
			// Store context layout name (preserved across view changes)
			contextLayoutName: string | null = null;

			constructor(view: HighlightingView) {
				this.view = view;
			}
			...
			process(): void {
				...
				if (!this.scope) {
					if (this.view.model) {
						...
						this.scope = this.view.entityType ?? null;
					}
				}
				if (!this.scope) {
					return;
			 ...
			 */
			setupHighlightingListeners(): void {
				...
				if (watchedFields.length > 0) {
					...
					watchedFields.forEach(field => {
						this.view.listenTo(this.view.model, `change:${field}`, (_model: unknown, value: unknown) => {
			...
			setupRenderHandlers(): void {
				...
				this.view.listenTo(this.view.model, 'change', (model: Model, options: ModelChangeOptions) => {
				...
				this.view.on('after:mode-change', (mode: string) => {
			 ...
			 */
			setupFieldHighlightingForFields(): void {
			 ...
			 */
			setupFieldViewHighlighting(fieldView: FieldView): void {
				if (!fieldView || !this.view.fieldHighlighter) {
					return;
				}

				// Add highlighting methods to field view
				fieldView.applyHighlighting = (): void => {
				...
				fieldView.clearHighlighting = (): void => {
					if (this.view.fieldHighlighter && this.view.model && this.scope && fieldView.name) {
				...
				fieldView.applyStyle = (style: Record<string, string>): void => {
					if (this.view.fieldHighlighter && this.view.model && this.scope && fieldView.name) {
				...
				fieldView.setBackgroundColor = (color: string) => fieldView.applyStyle!({backgroundColor: color});
				fieldView.setTextColor = (color: string) => fieldView.applyStyle!({color});
				fieldView.setBorder = (border: string) => fieldView.applyStyle!({border});
			 ...
			 */
			addHighlightingMethodsToView(): void {
				if (!this.view.fieldHighlighter) return;

				// Apply highlighting to a specific field
				this.view.highlightField = (fieldName: string): void => {
				...
				this.view.clearFieldHighlighting = (fieldName: string): void => {
					const fieldView = this.view.getFieldView ? this.view.getFieldView(fieldName) : null;
					if (fieldView) {
				...
				this.view.applyFieldStyle = (fieldName: string, style: Record<string, string>): void => {
					const fieldView = this.view.getFieldView ? this.view.getFieldView(fieldName) : null;
					if (fieldView) {
				...
				this.view.getFieldHighlightStyle = (fieldName: string, _directRules: unknown = null): Record<string, string> | null => {
			 ...
			 */
			invalidateCacheForField(changedField: string): void {
			 ...
			 */
			checkConditionReferencesField(conditionGroup: ConditionGroup | ConditionGroup[], fieldName: string): boolean {
				...
				if (typeof conditionGroup === 'object' && conditionGroup !== null) {
					...
					if (conditionGroup.value && Array.isArray(conditionGroup.value)) {
						return (conditionGroup.value as ConditionGroup[]).some(subCondition =>
			 ...
			 */
			applyFieldHighlightingForAffectedFields(changedField: string): void {
				if (!this.view.getFieldViews || !this.view.fieldHighlighter) {
					return;
				}
