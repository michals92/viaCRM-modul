import type TeamsFieldView from 'espocrm/src/views/fields/teams';
...
extend<TeamsFieldView>(Dep => class extends Dep {
	override getDetailLinkHtml(id: string, name?: string): string {
		...
		const teamStyles = (this.getHelper().getAppParam('teamStyles') ?? {}) as Record<string, TeamStyle>;
