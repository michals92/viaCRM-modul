import type LayoutRowsView from 'espocrm/src/views/admin/layouts/rows';
...
extend<LayoutRowsView>(Dep => class extends Dep {
	override template = 'viacrm:admin/layouts/rows';
	...
	processQuickSearch(text: string): void {
		...
		this.$el.find('.disabled.connected .cell').each((_: number, el: HTMLElement) => {
			const $el = $(el);
