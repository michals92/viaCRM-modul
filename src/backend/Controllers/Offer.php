...
class Offer extends Record
{
    public function postActionGeneratePdfWithAttachment(Request $request): array
    {
        ...
        // Check permissions
        $entity = $this->entityManager->getEntityById('Offer', $entityId);
        if (!$entity) {
            throw new NotFound('Offer not found');
        }
        
        if (!$this->acl->check($entity, 'read')) {
            throw new Forbidden('No read access to Offer entity');
        }
        ...
        try {
            ...
            $pdfData = $pdfService->generate('Offer', $entityId, $templateId);
            ...
            $GLOBALS['log']->error('PDF Generation Failed for Offer ' . $entityId . ': ' . $e->getMessage(), [
                'entityId' => $entityId,
