...
class Kanban
{
    ...
    public function getResult(): Result
    {
        ...
        foreach ($statusList as $status) {
            ...
            // Get custom kanban sorting from metadata
            $kanbanOrderBy = $this->metadata->get(['scopes', $this->entityType, 'kanbanOrderBy']);
            $kanbanOrderDirection = $this->metadata->get(['scopes', $this->entityType, 'kanbanOrder'], 'ASC');
            ...
            if ($this->userId && !$this->orderDisabled) {
                ...
                $queryBuilder = $this->entityManager
                    ...
                    ->order('COALESCE:(kanbanOrder.order, ' . ($this->maxOrderNumber + 1) . ')', 'ASC');

                // Add custom ordering if configured
                if ($kanbanOrderBy) {
                    $queryBuilder->order($kanbanOrderBy, strtoupper($kanbanOrderDirection));
                }

                $itemQuery = $queryBuilder->build();
            } else if ($kanbanOrderBy) {
                // Apply custom ordering even when user order is disabled
                $itemQuery = $this->entityManager
                    ->getQueryBuilder()
                    ->select()
                    ->clone($itemQuery)
                    ->order($kanbanOrderBy, strtoupper($kanbanOrderDirection))
                    ->build();
            }
