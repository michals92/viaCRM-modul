...
const build = async () => {
	...
	log.step('Building & copying Easy Email');
	await buildEasyEmail(join(BUILD, 'easy-email', 'editor.js'));
	await cpy(
		join(BUILD, 'easy-email', 'editor.js'),
		join(CLIENT_PATH, 'lib', 'easy-email'),
		{ flat: true },
	);
	await cpy(
		join(BUILD, 'easy-email', 'editor.css'),
		join(CLIENT_PATH, 'css', 'easy-email'),
		{ flat: true },
	);
	...
	if (lessContexts.length > 0) {
		log.step('Compiling Themes');
		...
		log.step('No themes (.less) files to compile');
	}
